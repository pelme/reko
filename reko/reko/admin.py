...
class PickupLocationsIterator(ModelChoiceIterator):
    def __iter__(self) -> Iterator[tuple[ModelChoiceIteratorValue | str, str]]:
        pickup_locations = self.queryset.prefetch_related("pickup__ring", "location").order_by(
            "-pickup__date", "pickup__ring", "start_time"
        )
...
class OrderProductInline(admin.TabularInline[OrderProduct, Order]):
    ...
    class Media:
        css = {"all": ("admin/css/hide_product_fk_actions.css",)}
...
class PickupLocationInline(admin.TabularInline[PickupLocation, Pickup]):
    ...
    class Media:
        css = {"all": ("admin/css/hide_location_fk_actions.css",)}
